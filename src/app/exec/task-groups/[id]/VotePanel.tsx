...
interface Props {
  taskGroupId: string;
  userId: string;
  canCreate: boolean; // isLeader || globalRole >= 4
}
...
function CreateVoteModal({ loading, error, onSubmit, onClose }: CreateModalProps) {
  ...
  function handleSubmit() {
    onSubmit({ title, description, options, closedAt });
  }
  ...
  function handleVoted(voteId: string, optionId: string) {
    setVotes((prev) =>
      prev.map((v) => {
        ...
        // Unmark previous vote, mark new one, adjust counts
