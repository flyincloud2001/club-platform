 ...
 *
 * Team / Role 對應 config.yaml 的 teams 與 roles 設定：
 *   teams: event | marketing | operation
...
/**
 * MemberTeam — 社團執行 team（對應 config.yaml teams[].id）
 * 目前三個 team：活動、行銷、營運
 */
export type MemberTeam = "event" | "marketing" | "operation";
...
export interface Member {
  ...
  /**
   * 所屬 team（對應 config.yaml teams[].id）
   * TODO: 替換為 Prisma User.teamId FK
   */
  team: MemberTeam;
...
/** teamLabel — Team ID → 中文顯示名稱（對應 config.yaml teams[].name） */
export const TEAM_LABELS: Record<MemberTeam, string> = {
  event: "Event Team",
  marketing: "Marketing Team",
  operation: "Operation Team",
};

/** teamLabelEn — Team ID → 英文顯示名稱 */
export const TEAM_LABELS_EN: Record<MemberTeam, string> = {
  event: "Event Team",
  marketing: "Marketing Team",
  operation: "Operation Team",
};
...
/**
 * getMembersByTeam — 依 team 篩選成員
 *
 * TODO: 替換為 prisma.user.findMany({ where: { teamId: team } })
 */
export function getMembersByTeam(team: MemberTeam): Member[] {
